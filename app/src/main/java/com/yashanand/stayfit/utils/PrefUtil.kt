package com.yashanand.stayfit.utils
...
class PrefUtil {
    ...
    companion object {
        ...
        fun getTimerState(context: Context): StartExercise.TimerState{
            val preferences = PreferenceManager.getDefaultSharedPreferences(context)
            val ordinal = preferences.getInt(TIMER_STATE_ID, 0)
            return StartExercise.TimerState.values()[ordinal]
        }

        fun setTimerState(state: StartExercise.TimerState, context: Context){
            val editor = PreferenceManager.getDefaultSharedPreferences(context).edit()
